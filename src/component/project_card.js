        ...
        <div class="col-lg-4 project-card-container mb-3">
            <Card project={this.state}>
                <div className="card-body">
                    ...
                    <Link to={this.state.url} className="btn btn-primary" target="_blank">Source</Link>
                    <ProjectDocsButton />
                </div>
            </Card>
        </div>
...
function ProjectTags(project) {
    ...
    return (
        <div>
            {project.map(tag => (
                <span class="badge badge-light filter-badge">{ project.tag }</span>
            ))}
        </div>
    );
}
...
function ProjectEulerSolutionsButton(project) {
    ...
    if (project.name.toLowerCase().includes("project euler")) {
        return (
            <Link to="/euler" className="btn btn-primary">Solutions</Link>
        );
    }
}
